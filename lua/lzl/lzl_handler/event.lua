...
---@alias LzlEvent {id:string, event:string[]|string, pattern?:string[]|string}
---@alias LzlEventSpec string|{event?:string|string[], pattern?:string|string[]}|string[]

---@class LzlEventHandler:LzlHandler
...
---@type table<string, LzlEvent>
M.mappings = {
	VeryLazy = { id = "VeryLazy", event = "User", pattern = "VeryLazy" },
	DeferredUIEnter = { id = "DeferredUIEnter", event = "User", pattern = "VeryLazy" },
	-- Example:
...
---@param spec LzlEventSpec
---@return LzlEvent
function M:_parse(spec)
	local ret = M.mappings[spec] --[[@as LzlEvent?]]
	if ret then
		return ret
	end
